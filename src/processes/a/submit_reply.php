...
try {
    if ($action_type === 'edit' && $reply_id > 0) {
        ...
        echo $_SESSION['success_message'] = 'Reply edited successfully.';
        if ($grade != 'general'){
            if($utype === "ADMIN"){
                header("Location: ../../../public/admin/post_view.php?grade=$grade&id=$post_id&edited_reply=$reply_id");
            } else if ($utype === "USER"){
                header("Location: ../../../public/user/post_view.php?grade=$grade&id=$post_id&edited_reply=$reply_id");
            }
        }else{
            if($utype === "ADMIN"){
                header("Location: ../../../public/admin/post_view.php?grade=$grade&id=$post_id&edited_reply=$reply_id");
        ...
        echo $_SESSION['success_message'] = 'Reply added successfully.';
        if ($grade != 'general'){
            if($utype === "ADMIN"){
                header("Location: ../../../public/admin/post_view.php?grade=$grade&id=$post_id&new_reply=$new_reply_id");
            } else if ($utype === "USER"){
                header("Location: ../../../public/user/post_view.php?grade=$grade&id=$post_id&new_reply=$new_reply_id");
            }
        }else{
            if($utype === "ADMIN"){
                header("Location: ../../../public/admin/post_view.php?grade=$grade&id=$post_id&new_reply=$new_reply_id");
